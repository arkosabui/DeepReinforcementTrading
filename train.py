...
parser.add_argument('--num_episode', action="store", dest="num_episode", default=21, type=int, help='episode number')
...
for e in range(1, num_episode + 1):
    ...
    for t in range(1, trading_period + 1):
        ...
        # execute position
        if t % 100 == 0:
            logging.info('Step: {}\tHold signal: {:.4} \tBuy signal: {:.4} \tSell signal: {:.4}'.format(t, actions[0],
                                                                                                        actions[1],
                                                                                                        actions[2]))
        ...
        if len(agent.memory) > agent.buffer_size:
            ...
            if t % 100 == 0:
                logging.info('Episode:{}\tLoss:{:.2f}\tAction:{}\tReward:{:.2f}\tBalance: {:.2f}\tNumber of Stocks: {}'
                         .format(e, loss, action_dict[action], reward, agent.balance, len(agent.inventory)))
